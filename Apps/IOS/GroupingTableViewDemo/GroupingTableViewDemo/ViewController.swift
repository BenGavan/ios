...
class ViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.rightBarButtonItem = UIBarButtonItem(title: "Group Last Name", style: .plain, target: self, action: #selector(handleGroup))
        
        navigationItem.leftBarButtonItem = UIBarButtonItem(title: "Group Age", style: .plain, target: self, action: #selector(handleGroupAge))
        
    }
    
    func changeBarButtonsToReset() {
        navigationItem.leftBarButtonItem?.title = "Reset"
        navigationItem.rightBarButtonItem?.title = "Reset"
    }
    
    func changeBarButtonsToOrig() {
        navigationItem.leftBarButtonItem?.title = "Group Age"
        navigationItem.rightBarButtonItem?.title = "Group Last Name"
    }
    
    @objc fileprivate func handleGroupAge() {
    ...
    @objc fileprivate func handleGroup() {
        if groupedPeople.count > 0 {
            groupedPeople.removeAll()
            tableView.reloadData()
            changeBarButtonsToOrig()
